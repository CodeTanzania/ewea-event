...
// constants
const COUNTRY_CODE = env.getString('COUNTRY_CODE', 'TZ');
const STAGE_ALERT = 'Alert';
const STAGE_EVENT = 'Event';
const STAGES = [STAGE_ALERT, STAGE_EVENT];
const MODEL_NAME = env.getString('EVENT_MODEL_NAME', 'Event');
const COLLECTION_NAME = env.getString('EVENT_COLLECTION_NAME', 'events');
...
const EventSchema = mongooseCommon.createSchema(
  {
    ...
    areas: {
      ...
      exportable: {
        format: v =>
          lodash.join(
            lodash.map(v, area => lodash.get(area, 'strings.name.en')),
            ', '
          ),
        default: 'NA',
      },
...
/* export event model */
const Event = mongooseCommon.model(MODEL_NAME, EventSchema);
