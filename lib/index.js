...
const sendEventNotification = (event, done) => {
  ...
  const message = common.parseTemplate(TEMPLATES_EVENT_NOTIFICATION_MESSAGE, {
...
const preLoadRelated = (optns, done) => {
  ...
  const ensureType = next => {
    const typeId = common.idOf(options.type) || options.type;
    if (typeId) {
      // TODO: or find default
      return predefine.Predefine.getById({ _id: typeId }, next);
    }
  ...
  const ensureGroup = next => {
    ...
    if (typeId) {
      ...
      return predefine.Predefine.getById({ _id: typeId }, (error, type) => {
        const group = lodash.get(type, 'relations.group');
...
const postEventWithChanges = (optns, done) => {
  ...
  return async.waterfall(
    [
      ...
      (related, next) => {
        const event = common.mergeObjects(options, related);
